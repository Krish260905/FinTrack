...
function LoginForm() {
  ...
  return (
    <>
      <div className="mb-6">
        <h2 className="text-3xl font-bold text-slate-700 mb-1">Login</h2>
      </div>

      {successMsg && (
        <div className="p-3 mb-4 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-600 text-sm flex items-center gap-2">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-1">
          <label className="text-sm text-slate-600 font-medium">Username/Email address <span className="text-red-500">*</span></label>
          <input 
            type="email" 
            required
      ...
      <div className="mt-4">
        <Link href="#" className="text-sm text-[#5c6cf1] hover:underline font-medium">Forgot password?</Link>
      </div>
  ...
  return (
    <div className="min-h-screen w-full flex flex-col md:flex-row bg-white relative overflow-hidden">
      ...
      <div className="w-full md:w-[45%] p-8 md:p-16 lg:p-24 flex flex-col pt-16 md:pt-32 text-white relative z-10 min-h-[300px] md:min-h-screen">
        <h1 className="text-5xl md:text-6xl font-bold mb-4 drop-shadow-sm">Welcome!</h1>
        <p className="text-lg md:text-xl text-blue-50 mb-10 font-medium drop-shadow-sm leading-snug">
          Create your account.<br/>For Free!
        </p>
        <Link href="/signup" className="inline-block border-2 border-white text-white rounded-full px-12 py-3 font-medium hover:bg-white hover:text-[#5c6cf1] transition-colors w-max shadow-sm text-lg">
          Sign Up
        </Link>
      </div>
