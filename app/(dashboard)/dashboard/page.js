...
import { Search, Activity, ArrowRight, SlidersHorizontal, ChevronDown, CreditCard, TrendingUp, Target, ArrowRightLeft, ReceiptText, LogOut, Calendar, Loader2 } from 'lucide-react';
...
const DashboardCharts = dynamic(() => import('@/components/DashboardCharts'), {
  ...
  loading: () => (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
      <div className="lg:col-span-3 dashboard-card p-6 h-[400px] flex items-center justify-center bg-slate-50/50 animate-pulse">
         <Loader2 className="w-8 h-8 text-slate-300 animate-spin" />
      </div>
      <div className="lg:col-span-2 dashboard-card p-6 h-[400px] flex items-center justify-center bg-slate-50/50 animate-pulse">
         <Loader2 className="w-8 h-8 text-slate-300 animate-spin" />
      </div>
    </div>
  )
...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Middle Charts */}
      <DashboardCharts data={data} timeframe={timeframe} formatCurrency={formatCurrency} COLORS={COLORS} />
